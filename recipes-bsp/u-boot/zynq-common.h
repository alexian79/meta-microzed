...
#define CONFIG_EXTRA_ENV_SETTINGS	\
	...
	"bootenv=uEnv.txt\0" \
	"bootenv_dev=usb\0" \
	"loadbootenv=load ${bootenv_dev} 0:0 ${loadbootenv_addr} ${bootenv}\0" \
	"importbootenv=echo Importing environment from ${bootenv_dev} ...; " \
		"env import -t ${loadbootenv_addr} $filesize\0" \
	"bootenv_existence_test=test -e ${bootenv_dev} 0:0 /${bootenv}\0" \
	...
	"jtagboot=echo TFTPing FIT to RAM... && " \
		...
		"bootm ${load_addr}\0" \
	"kernel_image=uImage-initramfs-microzed-zynq7.bin\0" \
	"kernel_load_address=0x2080000\0" \
	"devicetree_image=system.dtb\0" \
	"devicetree_load_address=0x2000000\0" \
	"usbboot=if usb start; then " \
			"echo XB booting from USB ... && " \
			"run usb_loadbootenv && " \
			"fatload usb 0:0 ${kernel_load_address} ${kernel_image} && " \
			"fatload usb 0:0 ${devicetree_load_address} ${devicetree_image} && " \
			"bootm ${kernel_load_address} - ${devicetree_load_address}; fi\0" \
		DFU_ALT_INFO \
		BOOTENV
#endif
